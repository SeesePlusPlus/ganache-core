  ...
  if (self.connected) {
    ...
    if (messageType === "request") {
      ...
      if (type === "step") {
        ...
        jsonData.specialEvent = null;

        if (jsonData.opcode.name === "JUMP" || jsonData.opcode.name === "JUMPI" || jsonData.opcode.name === "JUMPDEST") {
          // need to know about callstack and vunction parameter locations
          sendMessage = true;
          jsonData.specialEvent = "jump";
        }
        ...
        if (address in self.variableDeclarations && self.variableDeclarations[address].indexOf(pc) >= 0) {
          ...
          sendMessage = true;
          jsonData.specialEvent = "declaration";
        }
        ...
        if (!sendMessage && self.fastStep) { // haven't already decided to send message, so lets see if we're at a breakpoint
          ...
          for (i = 0; i < self.breakpoints.length; i++) {
            if (self.breakpoints[i].pc === pc && self.breakpoints[i].address === address) {
              sendMessage = true;
              jsonData.specialEvent = "breakpoint";
              break;
            }
          }
          if (i >= self.breakpoints.length) {
