  ...
  if (self.connected && self.connection != null) {
    ...
    self.internalEventEmitter.on("data", function handler(data) {
      ...
      if (data.id == msgId && data.messageType == "response") {
        ...
        callback(null, data.content); // TODO: add some extra stuff to standardize the format of this content
      }
    });
  }
  else {
...
SdbHook.prototype.linkDebugSymbols = function(sourceName, contractName, address, sourcePath, compilationResult) {
  var self = this;
  
  const cleanedAddress = address.replace("0x", "");

  self.compilationResult = compilationResult;
  self.compilationResult.sources[sourceName].lineBreaks = null;
  self.compilationResult.contractMap = {};
  self.compilationResult.contractMap[cleanedAddress] = sourceName + ":" + contractName;
  self.compilationResult.contracts[sourceName + ":" + contractName].address = cleanedAddress;
  self.compilationResult.contracts[sourceName + ":" + contractName].sourcePath = sourcePath;

  /*if (cleanedAddress in self.monitoredContracts) {
    self.monitoredContracts[cleanedAddress].sourceName = sourceName;
    self.monitoredContracts[cleanedAddress].sourceMap = sourceMap;
    self.monitoredContracts[cleanedAddress].sourcePath = sourcePath;
    self.monitoredContracts[cleanedAddress].lineBreaks = null;
    self.monitoredContracts[cleanedAddress].functionHashes = functionHashes;
  }
  else {
    self.monitoredContracts[cleanedAddress] = {
  ...
  self.trigger("monitoredContractsChanged", self.compilationResult, (err, responseContent) => {
    if (responseContent && "status" in responseContent && responseContent.status == "error") {
      console.log("cant send addresses because debugger isnt connected");
    }
    else {
