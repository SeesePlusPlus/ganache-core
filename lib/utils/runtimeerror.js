...
RuntimeError.prototype.combine = function(transactions, vm_output) {
  ...
  if (this.hashes.length == 1) {
    var error = this.results[this.hashes[0]].error;
    var errorMessage = typeof error === "object" ? JSON.stringify(error) : error;
    this.message = "VM Exception while processing transaction: " + errorMessage;
  } else {
    ...
    for (var i = 0; i < this.hashes.length; i++) {
      ...
      var error = this.results[hash].error;
      var errorMessage = typeof error === "object" ? JSON.stringify(error) : error;
      this.message += hash + ": " + errorMessage + "\n";
    }
  }
};
