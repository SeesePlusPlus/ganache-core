...
let ModuleInterface = require("./moduleInterface");
...
function Provider(options) {
  ...
  this.interface = new ModuleInterface(options);
  gethApiDouble.waitForInitialization(function(err, state) {
    if(err) {
      console.error(err); // TODO: @benjamincburns how would you prefer to handle this error?
    }
    else {
      self.interface.initialize(state.blockchain.vm);
    }
  })
