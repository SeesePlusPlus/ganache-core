...
StateManager = function(options, callback) {
  var self = this;
  ...
  if (options.fork) {
    ...
    this.blockchain = new BlockchainDouble(options, callback);
  }

  this.vm = this.blockchain.vm;
  this.sdbHook = this.blockchain.sdbHook;
  ...
  self.blockchain.getTransactionReceipt(tx_hash, function(err, receipt) {
    ...
    self.blockchain.latestBlock(function(err, block) {
      ...
      if (receipt.contractAddress != null) {
        ...
        if (self.sdbHook != null) {
          self.sdbHook.reportContractCreated(receipt.contractAddress);

          self.getCode(receipt.contractAddress, "latest", (err, code) => {
            if (err) return callback(err);
          })
        }
      }
...
StateManager.prototype.processGasEstimate = function (from, rawTx, callback) {
  ...
  if (self.sdbHook) {
    self.sdbHook.processingGasEstimate = true;
  }
  ...
  self.createFakeTransactionWithCorrectNonce(rawTx, from, function(err, tx) {
    ...
    self.blockchain.processCall(tx, function (err, results) {
      if (self.sdbHook) {
        self.sdbHook.processingGasEstimate = false;
      }
