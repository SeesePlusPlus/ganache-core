...
var VM = require('../../ethereumjs-vm'); // TODO: remove ../../
...
var SdbHook = require("./utils/sdbhook");

function BlockchainDouble(options, callback) {
  var self = this;
  ...
  this.sdbHook = options.sdb == true ? new SdbHook(options, callback) : null;
  ...
  this.data.initialize(function(err) {
    ...
    self.latestBlock(function(err, block) {
      ...
      if (options.debug == true) {
        self.vm.on('step', function(info, callback) {
          self.logger.log(info.opcode.name)
          if (options.sdb == true && self.sdbHook.isConnected()) {
            // send step event to debugger
            self.sdbHook.trigger('step', info, (err, data) => {
              if (err) {
                ...
                switch (data.content.type) {
                  case 'putCodeRequest':
                    ...
                    self.vm.stateManager.putContractCode(data.content.address, data.content.code, (err) => {
                      if (err) {
                        console.log("ERROR: " + err);
                      }
                      else {
                        self.sdbHook.trigger('response', {
                          'status': 'ok',
                          'id': data.id,
                          'mesageType': 'response',
                          'content': {
                ...
                // they obviously didn't care about the content, lets move on
                callback();
              }
            });
          }
          else {
            // if we're not using the debugger, just call the callback
            callback();
          }
        });
      }
